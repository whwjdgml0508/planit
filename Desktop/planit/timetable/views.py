...
class TimetableView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # 현재 학기의 시간표 슬롯만 가져오기
        if current_semester:
            semester_timeslots = TimeSlot.objects.filter(
                subject__user=user,
                semester=current_semester
            ).select_related('subject')
        else:
            # 학기가 없으면 학기 미지정 슬롯 표시
            semester_timeslots = TimeSlot.objects.filter(
                subject__user=user,
        ...
        # 해당 학기 시간표에 있는 과목들
        semester_subject_ids = semester_timeslots.values_list('subject_id', flat=True).distinct()
        semester_subjects = Subject.objects.filter(id__in=semester_subject_ids)
        ...
        # 시간표에 과목 배치 (현재 학기 TimeSlot만)
        for time_slot in semester_timeslots:
            ...
            for day in days:
                for period in periods:
                    TimeSlot.objects.create(
                        subject=subject,
                        semester=current_semester,  # TimeSlot에 학기 할당
                        day=day,
                        period=int(period),
                        location=location
                    )
            
            semester_info = f" ({current_semester} 시간표에 추가됨)" if current_semester else ""
...
class SemesterListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return Semester.objects.filter(user=self.request.user).order_by('-year', '-semester')
...
def add_to_timetable(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # 현재 학기 가져오기 (중복 체크 전에 필요)
            current_semester = Semester.objects.filter(
                user=request.user,
                is_current=True
            ).first()
            
            # 중복 시간 체크 (현재 학기에서 다른 과목과의 충돌만 체크)
            ...
            # 같은 과목이 이미 해당 시간에 있는지 체크 (현재 학기에서만)
...
class SemesterUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # 해당 학기의 과목 수 및 총 학점 (학점이 있는 과목만)
        subjects = Subject.objects.filter(user=self.request.user, semester=semester, credits__isnull=False)
        context['subject_count'] = subjects.count()
        total = subjects.aggregate(total=Sum('credits'))['total']
        if total is None:
            context['total_credits'] = 0
        else:
            # 소수점이 .0이면 정수로 표시
            context['total_credits'] = int(total) if total == int(total) else total
        return context
