...
# 2. accounts/views.py 파일 복사
Write-Host "📁 accounts/views.py 파일 복사 중..." -ForegroundColor Blue
ssh -i $SSH_KEY_PATH $EC2_USER@$EC2_IP "sudo cp /var/www/planit/accounts/views.py /home/ubuntu/planit/accounts/views.py"

# 3. 템플릿 파일 복사
Write-Host "📁 템플릿 파일 복사 중..." -ForegroundColor Blue
ssh -i $SSH_KEY_PATH $EC2_USER@$EC2_IP "sudo cp /var/www/planit/templates/accounts/profile.html /home/ubuntu/planit/templates/accounts/profile.html"
...
ssh -i $SSH_KEY_PATH $EC2_USER@$EC2_IP "sudo systemctl status planit --no-pager -l"
